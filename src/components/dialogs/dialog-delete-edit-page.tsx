...
import { useDeleteClient } from "@/hooks/client/use-delete-client";
import { usePets } from "@/hooks/pets/use-pets";
import Image from "next/image";
import { useRouter } from "next/navigation";
import { toast } from "react-toastify";

interface IDialogDeleteEditPageProps
...
export function DialogDeleteEditPage({
  onClose,
  page,
  entity,
  id,
}: IDialogDeleteEditPageProps) {
  const { refetch } = useClients(1, "");
  const { refetch: refetchPets } = usePets(1, "");
  const router = useRouter();

  const { mutateAsync, isLoading } = useDeleteClient(id);
